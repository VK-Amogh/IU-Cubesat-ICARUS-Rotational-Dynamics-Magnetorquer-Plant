...
def plot_magnetic_underactuation(
    ...
    *,
    ...
    axes[0].plot(angle_deg, torque_norm * 1e6, label="Simulated |m x B|")
    axes[0].plot(
        angle_deg,
        expected_norm * 1e6,
        "--",
...
def run_verification_suite(output_dir: Path | str) -> dict[str, object]:
    ...
    generated_files = {path.name for path in output_path.iterdir() if path.is_file()}
    generated_files.update({"verification_report.md", "verification_summary.json"})
